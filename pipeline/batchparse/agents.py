...
def extract(data: bytes) -> list[bytes]:
    ...
    # 检查数据长度是否符合最小要求
    if len(data) < 2561:
        raise ValueError(f"数据长度不足，期望至少2561字节，实际{len(data)}字节")

    # 从字节偏移124读取COMID数量（1字节）
    comid_count_offset = 124
    comid_count = data[comid_count_offset]
    ...
    for i in range(comid_count):
        # 计算车厢数据报文头和数据帧的起始地址
        header_start = 133 + i * 404  # 每节车厢间隔404字节（4字节报文头 + 400字节数据帧）
        frame_start = header_start + 4  # 数据帧紧跟在报文头后

        # 提取车厢数据帧（400字节）
        frame_data = data[frame_start: frame_start + 400]

        # 验证数据帧长度
        if len(frame_data) != 400:
            raise ValueError(f"车厢{i + 1}的数据帧长度不足，期望400字节，实际{len(frame_data)}字节")
