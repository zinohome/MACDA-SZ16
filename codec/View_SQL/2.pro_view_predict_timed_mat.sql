CREATE MATERIALIZED VIEW pro_view_predict_timed_mat AS  -- 视图名修改为pro_view_predict_timed
WITH
...
event_boundaries AS (
    SELECT
        ...
        dvc_carriage_no,
        param_name,
        msg_calc_dvc_time AS event_time,  -- 时序字段（与原表一致）
    ...
    FROM pro_predict_transposed  -- 原表名修改为pro_predict_transposed
    WHERE
...
predict_ends AS (  -- 逻辑与error_ends一致，重命名为predict_ends
    SELECT
        ...
        MIN(e.msg_calc_dvc_time) AS predict_end_time  -- 结束时间字段名调整为predict_end_time
    FROM predict_starts s
    LEFT JOIN pro_predict_transposed e  -- 关联原表pro_predict_transposed
        ...
        AND e.msg_calc_dvc_time > s.predict_start_time  -- 结束时间在预测开始后
        AND e.msg_calc_dvc_time <= s.predict_start_time + INTERVAL '30 minutes'  -- 30分钟窗口内
    GROUP BY
