CREATE MATERIALIZED VIEW dev_view_error_timed_mat AS
WITH
...
event_boundaries AS (
    SELECT
        ...
        dvc_carriage_no,
        param_name,
        msg_calc_parse_time AS event_time,  -- 原始表时间字段的别名
        param_value,
...
error_starts AS (
    SELECT
        ...
        dvc_carriage_no,
        param_name,
        event_time AS error_start_time  -- 来自event_boundaries的event_time（即msg_calc_parse_time）
...
error_ends AS (
    SELECT
        ...
        s.param_name,
        s.error_start_time,
        -- 改为使用原始表的msg_calc_parse_time字段（原event_time是别名）
        MIN(e.msg_calc_parse_time) AS error_end_time
    FROM error_starts s
    LEFT JOIN dev_error_transposed e  -- 关联原始表（非event_boundaries）
        ...
        AND e.msg_calc_parse_time > s.error_start_time  -- 结束时间在故障开始后
        AND e.msg_calc_parse_time <= s.error_start_time + INTERVAL '30 minutes'  -- 30分钟窗口内
    GROUP BY
