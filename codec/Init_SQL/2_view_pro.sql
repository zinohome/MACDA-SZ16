-- 创建视图：pro_view_train_carriage
CREATE OR REPLACE VIEW pro_view_train_carriage AS
SELECT DISTINCT
    dvc_train_no,
    dvc_carriage_no
FROM
...
-- 创建视图：pro_view_param
CREATE OR REPLACE VIEW pro_view_param AS
SELECT
    msg_calc_dvc_time,
    msg_calc_parse_time,
    msg_calc_dvc_no,
...
-- 创建视图：pro_view_error
CREATE OR REPLACE VIEW pro_view_error AS
SELECT
    msg_calc_dvc_time,
    msg_calc_parse_time,
    msg_calc_dvc_no,
...
-- 创建视图：pro_view_statistic
CREATE OR REPLACE VIEW pro_view_statistic AS
SELECT
    msg_calc_dvc_time,
    msg_calc_parse_time,
    msg_calc_dvc_no,
...
-- 创建视图：pro_view_param_transposed（参数类型字段转置）
CREATE OR REPLACE VIEW pro_view_param_transposed AS
SELECT
    p.msg_calc_dvc_time,
...
-- 创建视图：pro_view_error_transposed（错误类型字段转置）
CREATE OR REPLACE VIEW pro_view_error_transposed AS
SELECT
    e.msg_calc_dvc_time,
...
-- 创建视图：pro_view_statistic_transposed（统计类型字段转置）
CREATE OR REPLACE VIEW pro_view_statistic_transposed AS
SELECT
    s.msg_calc_dvc_time,
