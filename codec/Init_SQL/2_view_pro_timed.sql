...
WITH filtered_data AS (
    ...
    SELECT
        ...
        dvc_carriage_no,
        param_name,
        msg_calc_dvc_time AT TIME ZONE 'Asia/Shanghai' AS event_time,  -- 转换为上海时间
        param_value
    FROM
        pro_mview_error_transposed
    WHERE
        msg_calc_dvc_time >= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Shanghai') - INTERVAL '7 days'  -- 近7天
    ORDER BY
        param_name, msg_calc_dvc_time  -- 提前排序
),
status_changes AS (
    SELECT
        msg_calc_dvc_no,
