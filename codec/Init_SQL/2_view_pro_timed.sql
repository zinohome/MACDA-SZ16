-- 创建视图 pro_mview_error_changes
-- 创建预计算状态变化物化视图（每日刷新）
CREATE MATERIALIZED VIEW IF NOT EXISTS pro_mview_error_changes AS
...
-- 添加必要的索引
CREATE INDEX idx_pro_error_changes ON pro_mview_error_changes (
    param_name, event_time, is_fault, is_change_point
);



-- 创建视图 pro_view_error_timed
-- 创建快速查询视图
CREATE OR REPLACE VIEW pro_view_error_timed AS
WITH
-- 1. 从预计算视图获取数据（仅最近7天）
recent_changes AS (
    SELECT *
    FROM pro_mview_error_changes
