...
FROM (
    ...
    LEFT JOIN pro_view_fault_timed fault
        ON train.dvc_train_no = fault.dvc_train_no
        AND train.dvc_carriage_no = fault.dvc_carriage_no
        AND (fault.status = '持续中')
    GROUP BY train.dvc_train_no
...
WITH carriage_status AS (
    SELECT
        ...
        CASE
            ...
            WHEN BOOL_OR(dvft.fault_type = '预测') THEN 'planned_maintenance'
            ELSE 'normal'
        END AS status
    FROM
        dev_macda_ac dt
    LEFT JOIN
        dev_view_fault_timed dvft
